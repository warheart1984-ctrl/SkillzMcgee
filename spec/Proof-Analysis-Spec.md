...
- **CAR-1.0** (read-only) â€” canonical artifact registry for identity and lineage context
- **COR-1.0** state vector (read-only) â€” structural integrity and requirement groupings
- **DRA-1.0** (read-only, when available) â€” dependency-risk and readiness metrics
...
- â€œWhat breaks if X disappears?â€
- â€œWhat regresses if Y changes?â€
...
- DRA-1.0: `tools/generators/dra-analyze.mjs` â†’ `meta/DRA-1.0.json`

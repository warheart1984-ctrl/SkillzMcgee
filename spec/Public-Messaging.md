...
- **Evidence-first** — Claims are backed by artifacts registered in CAR, not authority.
- **Reproducibility as legitimacy** — Independent reproduction is the apex maturity state.
- **Deterministic observability** — COR reports the same state given the same CAR inputs.
- **Separation of concerns** — Canonical ≠ validation ≠ measurement ≠ analysis ≠ governance.
- **Governance as verification, not trust** — Stewards decide; they do not measure or validate.

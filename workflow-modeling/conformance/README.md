# WMC-CTS-1.0 — Workflow Modeling Conformance Test Suite

**Version:** 1.0 · **Suite:** [WMS-1.0](../README.md)
...
|-----------------|------|----------|
| §3.1 No rec without obs | `validateCanvas` CMS-1 | `validation.issues[]` empty |
| §3.2 No finding without obs | `validateCanvas` CMS-2 | finding.observationIds |
| §3.4 No metric without outcome | `validateCanvas` CMS-4 | metric.expectedOutcomeIds |
| §5 Traceability | chain completeness | traceabilityMap.chains |
| §6 Reproducibility | same input → same validate | CI workflow-canvas:validate |
...
W0 is implemented. W1–W3 profiles are specified; W2/W3 integrate with Nova Studio operator review and CRK-1 receipt registration (future).
